...
class Create extends Component
{
    ...
    private function saveContact($redirectAfterSave = true)
    {
        ...
        try {
            ...
            if ($redirectAfterSave) {
                ToastHelper::flashSuccess('The supplier was successfully registered.', 'Saved');
                return redirect()->route($this->redirectRoute);
            } else {
                $this->showToastSuccess('The supplier was successfully registered.', 'Saved');
            }
        } catch (\Exception $e) {
            DB::rollBack();
            $this->showToastError('An error occurred while saving the supplier. ' . $e->getMessage());
        }
    }
    ...
    {
        $this->authorize('create', Supplier::class);
        return view('livewire.contacts.supplier.create');
    }
}
