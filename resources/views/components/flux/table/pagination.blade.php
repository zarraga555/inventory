...
@if ($paginator->hasPages())
    <div class="pt-3 border-t border-zinc-100 dark:border-zinc-700 flex justify-between items-center gap-3"
        ...
        <div
            ...
            @if ($paginator->previousPageUrl())
                <button wire:click="previousPage"
                    class="flex justify-center items-center size-6 rounded-[6px] text-zinc-300 dark:text-zinc-500"
                    aria-label="« Previous">
                    <x-ui.icon name="chevron-left" class="shrink-0 size-4" />
                </button>
            @else
            ...
            @foreach ($paginator->links()->elements[0] as $page => $url)
                @if ($page == $paginator->currentPage())
                    <span
                        class="flex justify-center items-center size-6 rounded-[6px] bg-primary text-primary-foreground text-sm">{{ $page }}</span>
                @else
                    <button wire:click="gotoPage({{ $page }})"
                        class="flex justify-center items-center size-6 rounded-[6px] hover:bg-zinc-100 dark:hover:bg-white/20 text-zinc-400 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-white text-sm">
                        {{ $page }}
                    </button>
                @endif
            @endforeach
            ...
            @if ($paginator->hasMorePages())
                <button wire:click="nextPage"
                    ...
                    aria-label="Next »">
                    <x-ui.icon name="chevron-right" class="shrink-0 size-4" />
                </button>
            @else
                <span class="flex justify-center items-center size-6 rounded-[6px] text-zinc-300 dark:text-zinc-500">
                    <x-ui.icon name="chevron-right" class="shrink-0 size-4" />
