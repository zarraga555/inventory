<flux:modal name="show-changes" variant="flyout">
    <div class="w-full max-w-sm sm:w-[95vw] mx-auto p-4 overflow-y-auto overflow-x-hidden break-words max-h-[90vh]">
        ...
        <flux:heading size="lg" class="text-primary-600">
            {{ __('Log Details') }}
        </flux:heading>
        ...
        @if (!empty($selectedChanges))
            <div class="space-y-4 text-sm text-gray-800">
                ...
                @if (!empty($selectedChanges['action']))
                    <div class="bg-gray-100 border-l-4 border-primary-500 px-4 py-2 rounded shadow-sm">
                        <span class="text-sm font-medium text-gray-600">{{ __('Action performed') }}:</span>
                        <span class="ml-2 capitalize font-semibold text-primary-700">
                ...
                @if (!empty($selectedChanges['entity']))
                    <div class="bg-gray-50 border-l-4 border-indigo-500 px-4 py-2 rounded shadow-sm">
                        <span class="text-sm font-medium text-gray-600">{{ __('Entity') }}:</span>
                        <span class="ml-2 capitalize text-indigo-700">
                ...
                @if (!empty($selectedChanges['before']))
                    <div class="pl-4 border-l-4 border-yellow-500">
                        <h3 class="font-semibold text-yellow-600 mb-2">{{ __('Before Changes') }}:</h3>
                        <div class="space-y-1">
                ...
                @if (!empty($selectedChanges['after']))
                    <div class="pl-4 border-l-4 border-green-500">
                        <h3 class="font-semibold text-green-600 mb-2">{{ __('After Changes') }}:</h3>
                        <div class="space-y-1">
                ...
                @if (!empty($selectedChanges['performed_by']))
                    <div class="pl-4 border-l-4 border-gray-400">
                        <h3 class="font-semibold text-gray-600 mb-2">{{ __('Performed By') }}:</h3>
                        <div class="space-y-1">
